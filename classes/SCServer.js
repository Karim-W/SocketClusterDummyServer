...
module.exports = class SCServer {
  ...
  async disconnectionListener() {
    for await (let { socket, code, reason } of this.sc_server.listener(
      "disconnection"
    )) {
      console.log(
        `socket ${socket.id} disconnected with code ${code} due to ${reason}`
      );
    }
  }
  ...
  async outboundMiddlewareHandler(middlewareStream) {
    ...
    for await (let action of middlewareStream) {
      ...
      if (
        this.originatorCheck(action.socket.authToken.client_name, action.data)
      ) {
        action.allow();
        continue;
      }
  ...
  async connectToServer(host, port, subscriptions) {
    ...
    subscriptions.forEach((subscription) => {
      if (subscription.publish === true) {
        this.exchangeSubscriptionHandler(
  ...
  setOriginator(originator_name, data) {
    ...
    if (
      typeof data !== "object" ||
      (typeof data === "object" && !("originators" in data))
    ) {
      new_data = {
        originators: {
          [originator_name]: 1,
